...
class RiverDatabase(object):
    ...
    def connect_pg(self):
        ...
        try:
            ...
            msg = "Error! Can't connect to PostGIS database. Check connection details! -- %s"
            logging.error(msg, e, exc_info=1)
            msg = msg % repr(e)
        finally:
            logging.info(msg)
            return msg
    ...
    def disconnect_pg(self):
        ...
        if self.con:
            ...
            logging.error("Cannot disconnect. There is no opened connection!")
    ...
    def run_query(self, qry, fetch=False, arraysize=0, be_quiet=False):
        ...
        try:
            ...
            msg = "Error! -- %s"
            logging.error(msg, e, exc_info=1)
            logging.error("Query :: {}".format(qry))
        finally:
            return result
    ...
    def register_object(self, obj):
        ...
        if key not in self.register:
            ...
            logging.debug("{0} already exists inside RiverGIS registry.".format(key))
    ...
    def register_existing(self, hydro_module, schema=None, srid=None):
        ...
        for tab in tabs:
            if tab in dir(hydro_module):
                ...
                obj = hydro_object()
                self.register_object(obj)
                logging.debug("{0} registered".format(obj.name))
            else:
                pass
    ...
    def import_layer(self, gdf, fields, attr_map, selected):
        """
        Import a vector layer's features with attributes specified inside attr_map dictionary.

        Args:
            gdf (GeoDataframe): Source geopandas GeoDataframe containing the features to insert.
        ...
        """
        features = list(gdf.geometry.apply(lambda x: x.wkt))
        field_names = [c for c in gdf.columns if c != "geometry"]
    ...
    def layer_to_pgsql(self, features, fields, hecobject, schema, srid):
        ...
        for feat in features:
            ...
            if not target_multi and src_multi:
                ...
                if is_single:
                    ...
                    logging.warning("Source geometry is reported as MULTIPART but target is SINGLEPART! skipping...")
                    continue
        ...
        if single2multi is True:
            logging.warning("Source geometry is SINGLEPART but the target is MULTIPART!")
        else:
            pass
        return qry
    ...
    def create_schema(self, schema_name):
        ...
        if self.run_query(qry) is None:
            ...
            logging.info('Schema "{0}" created.'.format(schema_name))
    ...
    def drop_schema(self, schema_name, cascade=False):
        ...
        if self.run_query(qry) is None:
            ...
            logging.info('Schema "{0}" deleted.'.format(schema_name))
